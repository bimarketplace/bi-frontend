...
export default function ChatLayoutClient({ children }: { children: React.ReactNode }) {
    ...
    return (
        <div className="min-h-screen bg-[#FAFAFA] pt-28 pb-8 flex flex-col">
            <Container className="flex-1 flex flex-col h-[calc(100vh-9rem)] max-h-[850px]">
                <div className="bg-white rounded-[32px] overflow-hidden border border-gray-100 shadow-xl shadow-zinc-200/50 flex flex-1 w-full h-full relative">
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex justify-between items-center gap-2">
                                                    <p className={`text-sm truncate font-medium ${conv.unread_count > 0 ? 'text-zinc-900 font-bold' : 'text-zinc-500'}`}>
                                                        {conv.last_message ? (conv.last_message.sender.username === currentUsername ? `You: ${conv.last_message.body}` : conv.last_message.body) : (conv.product ? `Re: ${conv.product.name}` : "Started a conversation")}
                                                    </p>
                                                    {conv.unread_count > 0 && (
