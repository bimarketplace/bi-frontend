...
export default function Products({ 
  ...
  useEffect(() => {
      const fetchInitialData = async () => {
        // If we already have products for this vendor, don't refetch
        if (products.length > 0 && products[0].seller?.username === id) {
        ...
        try {
          ...
          const filteredResults = productsData.results.filter(
            (p: ProductType) => p.seller?.username === id
          );
          ...
          setPageError("Failed to load store products. Please refresh.");
        } finally {
          setIsInitialLoading(false);
        }
      };
